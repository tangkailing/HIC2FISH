...
All example inputs and experimental reference quantities in this directory were derived from the processed arrays in [`data/processed`](../processed/). Specifically, `example_hic.npy` was selected from the processed Hi-C conditioning matrices, `example_dna_fish_centroid_um.npy` was calculated by averaging the corresponding experimental single-cell DNA-FISH distance matrices, and `normalization_scalers.npz` was constructed from the training-partition Hi-C and DNA-FISH min–max parameters.
